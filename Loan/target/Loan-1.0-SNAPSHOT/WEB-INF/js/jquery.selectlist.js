    ...
    SelectList.prototype={
        ...
        renderSelect: function(element){
            var that = this,
                ...
                selectLength = that.getOptionCount($this),
                selectHTML = '<div id="' + selectID + '" class="select-wrapper ' + selectClassName + '"><input type="hidden" id="qwet'+selectID+'" name="' + selectName + '" value="' + selectedValue + '"/><i class="icon select-down"></i><input type="button" class="select-button" value="' + selectedOptionText + '"/><div class="select-list"><ul id="'+selectID+'1">',
                selectListHTML = '';
        ...
        setSelectEvent: function(element){
            ...
            if(that.settings.enable){
                ...
                $(selectID + ' li').on('click',function(event){
                    var sqw=$(this).text();
                    $.post("fastJSON", {user: sqw}, function (data) {
                        var total = data.l;
                        var user = data.user;
                        $('#salary1').empty();
                        for (i=0;i<total.length;i++){
                            $("#salary1").append('<li id="ss'+i+'" data-value="'+total[i]+'">'+total[i]+'</li>');
                            $('li[id="ss'+i+'"]').click(function(){
                                $(this)
