...
public interface VipCenterDao {
    //分页获取积分商品
    List<PointMall> getPointMall( @Param("start") Integer start,
    ...
    //查询成长经历生成list分页
    List<GrowthValue> getGrowthValueByUser(@Param("userId")int userId,
    ...
    //根据表名分页获得列表数据
    List<Map> getDataList(@Param("tableName")String tableName,
                          @Param("start")Integer start,
                          @Param("limit")Integer limit);
    ...
    //查询某些东西获得count
    int selSomethingGetCount(@Param("conditional")String conditional,
