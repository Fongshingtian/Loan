
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<html lang="en"><head>
    <meta charset="UTF-8">
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>小牛普惠-发展历程</title>
    <meta name="Keywords" content="小牛普惠公司动态,小牛普惠新闻公告,小牛普惠信贷动态">
    <meta name="Description" content="小牛普惠信贷信息动态中心,主要是关于小牛普惠公司的公司新闻、小牛普惠新闻公告和信贷、网贷信息资讯更新.">

    <!-- 页面自定义名称 -->




    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <!-- <meta name="viewport" content="width=device-width, initial-scale=1"> -->

    <!-- 是否为开发模式 -->

    <!-- 资源加载版本号 -->



    <!--[if lt IE 9]><script src="https://www.xnph66.com/HomePage/resources/xnph/vender/bootstrap/dist/js/ie8-responsive-file-warning.js"></script><![endif]-->
    <script src="https://hm.baidu.com/hm.js?d249a44e45d00944e513d824188f2185"></script><script src="https://www.xnph66.com/HomePage/resources/xnph/vender/bootstrap/dist/js/ie-emulation-modes-warning.js"></script>

    <!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
    <!--[if lt IE 9]>
    <script src="http://cdn.bootcss.com/html5shiv/3.7.0/html5shiv.js"></script>
    <script src="http://cdn.bootcss.com/respond.js/1.4.2/respond.min.js"></script>
    <![endif]-->



    <!-- common -->



    <link rel="stylesheet" type="text/css" href="https://www.xnph66.com/HomePage/resources/dist/build/build.css?v=2.1.2">


    <link rel="stylesheet" type="text/css" href="https://www.xnph66.com/HomePage/resources/dist/css/less/about_style.css?v=2.1.2">




    <!--[if lt IE 9]>
    <script src="http://html5media.googlecode.com/svn/trunk/src/html5media.min.js"></script>
    <![endif]-->


    <!--头部预先加载js-->
    <!-- common -->




    <script type="text/javascript" src="https://www.xnph66.com/HomePage/resources/dist/build/build.js?v=2.1.2"></script><link type="text/css" rel="stylesheet" href="https://www.xnph66.com/HomePage/resources/dist/build/need/laydate.css"><link type="text/css" rel="stylesheet" href="https://www.xnph66.com/HomePage/resources/dist/build/skins/default/laydate.css" id="LayDateSkin">



    <!-- 百度统计-->
    <script type="text/javascript">
        var XnphVideoPlaying = function(){console.log('nothing');};
    </script>


    <script>
        var _hmt = _hmt || [];
        (function() {
            var hm = document.createElement("script");
            hm.src = "https://hm.baidu.com/hm.js?d249a44e45d00944e513d824188f2185";
            var s = document.getElementsByTagName("script")[0];
            s.parentNode.insertBefore(hm, s);
        })();

        var IsPlay = false;
        XnphVideoPlaying = function() {

            if(!IsPlay && _hmt){
                _hmt.push(['_trackEvent', 'XNPH_VIDEO', 'play', 'white-collar workers']);
                IsPlay = true;
                console.log('baidu _trackEvent video play');
            }

        }
    </script>


    <!-- https不支持腾讯在线客服js，建议使用图片或者文字超链接发起在线咨询 -->
    <!-- <script charset="utf-8" type="text/javascript" src="https://wpa.b.qq.com/cgi/wpa.php"></script> -->






    <style type="text/css">
        .about-history-right-content {
            padding: 30px 80px 90px 10px;
        }
    </style>
</head>

<body>
<!--头部-->


<input type="hidden" value="https://www.xnph66.com" id="txt-base-url">
<input type="hidden" value="https://www.xnph66.com/HomePage/resources" id="txt-resource-url">
<input type="hidden" value="https://website-img.xnph66.com/" id="txt-img-url">
<!-- <header>
<div class="g-m">
    <ul>
        <li class="tel fl">全国服务热线：400-8730-666</li>
        <li class="wechat fr">
            <a href="javascript:;">微信关注我们</a>
            <img class="header-qrcode" src="https://www.xnph66.com/HomePage/resources/xnph/images/header-qrcode.png">
        </li>
        <li class="linkus fr"><a href="https://www.xnph66.com/contact">联系我们</a></li>
    </ul>
</div>
</header> -->
<nav class="g-m nav-wrap">
    <a href="https://www.xnph66.com"><img src="https://www.xnph66.com/HomePage/resources/xnph/images/xnph-jf-logo.jpg"></a>
    <a class="xn-nav txt-sty-m" data-menukey="index" href="javascript:window.location.href=location.host.indexOf('.com') == -1 ? '/HomePage/index' : '/'" style="margin-left: 30px">首&nbsp;&nbsp;页</a>
    <a class="xn-nav txt-sty-m" data-menukey="product" href="https://www.xnph66.com/product1">产品介绍</a>
    <a class="xn-nav txt-sty-m" data-menukey="loanapply" href="https://www.xnph66.com/loanapply">在线申请</a>
    <a class="xn-nav txt-sty-m" data-menukey="branch" href="https://www.xnph66.com/branch/1">营业网点</a>
    <a class="xn-nav pos-abt txt-sty-m active" data-menukey="about" href="https://www.xnph66.com/company">关于我们</a>
</nav>
<!--右侧停靠 begin-->
<div class="BizQQWPA_wrap fix-r-area">
    <div class="fra-wrap">
        <a class="f-btn-ic ic-customer-feedback" href="https://www.xnph66.com/user_feedback"></a>
        <!-- <a class="f-btn-ic ic-qq-server" id="BizQQWPA"></a> -->
        <a class="f-btn-ic ic-wechat-server"><i></i></a>
        <a class="f-btn-ic ic-back-top" href="javascript:document.documentElement.scrollTop = document.body.scrollTop = 0;" style="display: none;"></a>
    </div>
</div>
<!--右侧停靠 end-->
<div class="box-gry box-paddingb">
    <div class="crumbs">
        <a href="https://www.xnph66.com">首页&nbsp;</a>&gt;
        <a href="https://www.xnph66.com/newslistrows4">关于我们</a>
    </div>
    <div class="body-wrap">
        <div class="wrap-left fl">
            <div class="wrap-left-nav">
                <div class="wrap-left-nav-main">
                    <ul>
                        <li class="bg-gray-lter">
                            <a href="javascript:;" class="block-a">
                                <span class="l-n-title">关于我们</span>
                                <div class="wrap-left-nav-mask"></div>
                            </a>
                        </li>
                        <li class="">
                            <a href="https://www.xnph66.com/company" class="block-a">
                                <span class="l-n-title">公司介绍</span>
                                <div class="wrap-left-nav-mask"></div>
                            </a>
                        </li>
                        <li class="current">
                            <a href="https://www.xnph66.com/history" class="block-a">
                                <span class="l-n-title">发展历程</span>
                                <div class="wrap-left-nav-mask" style="display: block"></div>
                            </a>
                        </li>
                        <li class="">
                            <a href="https://www.xnph66.com/newslistrows4" class="block-a">
                                <span class="l-n-title">新闻公告</span>
                                <div class="wrap-left-nav-mask"></div>
                            </a>
                        </li>
                        <li class="">
                            <a href="https://www.xnph66.com/contact" class="block-a">
                                <span class="l-n-title">联系我们</span>
                                <div class="wrap-left-nav-mask"></div>
                            </a>
                        </li>
                        <li class="">
                            <a href="http://sc.hotjob.cn/wt/neocapital/web/index/social" target="_blank" class="block-a">
                                <span class="l-n-title">加入我们</span>
                                <div class="wrap-left-nav-mask"></div>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="wrap-right fr">
            <div class="wrap-right-title">
                发展历程
            </div>
            <div class="about-history-right-content">
                <ul>
                    <li class="clearfix">
                        <span class="graydots"></span>
                        <p class="text-orange">2018年小牛普惠在路上，再出发</p>
                    </li>

                    <li class="clearfix">
                        <span class="yellowcircle"></span>
                        <p class="time-year">2017年</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">9月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508745310330930.png">
                        <p>“走过风雨，遇见彩虹”2017年小牛普惠全国大会顺利召开 </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">6月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508745232073804.png">
                        <p>小牛普惠启动“爸爸很牛”公益计划，首期资助百余位脑瘫儿父亲。</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">6月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508745066821085.jpg">
                        <p>小牛资本董事长前往重庆，与小牛普惠重庆员工共同庆祝小牛走进新五年。</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">5月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508743634352896.png">
                        <p>小牛资本董事长彭铁前往北京、宁波“走基层”，宴请全国标杆团队，并展开一系列业务交流会。
                        </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">4月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508743338888284.png">
                        <p>小牛普惠全国巡讲会拉开序幕，小牛金服CEO王洁凤、副总裁王芳率多名高管“走基层”，从深圳出发，奔赴福州、合肥、郑州、成都、长沙、北京等地，深入了解全国各地基层业务员的工作与生活。
                        </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">2月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508742925961745.png">
                        <p>
                            小牛普惠与中行、工行、友邦保险等在《晶报》第六届“百万读者金融业口碑榜”评选中，同获“百万读者金融业口碑奖”。</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">1月</span>
                        <span class="graydots"></span>
                        <p>
                            小牛普惠携手多家同业名企共创风控协作联盟。
                        </p>
                    </li>

                    <li class="clearfix">
                        <span class="yellowcircle"></span>
                        <p class="time-year">2016年</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">12月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508742732387587.png">
                        <p>
                            小牛普惠受深圳市关爱行动公益基金会授牌殊荣。

                        </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">12月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508742155341435.png">
                        <p>
                            小牛普惠受邀参加中国小额信贷国际峰会，资产端创新模式受业界关注。

                        </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">10月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1508741957420601.png">
                        <p>
                            《小牛资本家庭金融战略暨小牛金服启动发布会》10月28日在深圳蛇口价值工厂隆重举行。       	</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">3月</span>
                        <span class="graydots"></span>
                        <p>                            	3月15日第一届小牛普惠信任节举行。
                        </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">1月</span>
                        <span class="graydots"></span>
                        <p>
                            2016年1月小牛普惠业绩突破10亿元！
                        </p>
                    </li>

                    <li class="clearfix">
                        <span class="yellowcircle"></span>
                        <p class="time-year">2015年</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">12月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1473074765756192.jpg">
                        <p>因在推进普惠金融发展、服务小微金融上表现突出，小牛普惠荣膺《晶报》“百万
                            读者金融口碑榜”--最佳小微金融服务奖。


                        </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">11月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1473074754700887.jpg">
                        <p>因在产品、服务和风控建设上的显著创新，小牛普惠荣登“2015中国普惠金榜”--金融业务创新奖；                            	                            	</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">7月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1473074670257455.jpg">
                        <p>小牛普惠投资管理有限公司第一百家营业部于辽宁沈阳盛大开业。
                        </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">1月</span>
                        <span class="graydots"></span>
                        <p>小牛普惠投资管理有限公司第十家营业部在广州天河东路盛大开业。
                        </p>
                    </li>

                    <li class="clearfix">
                        <span class="yellowcircle"></span>
                        <p class="time-year">2014年</p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">11月</span>
                        <span class="graydots"></span>
                        <img class="fl m-r-md w-sm" src="https://website-img.xnph66.com/static/pics/products/1473074349476549.jpg">
                        <p>随着小牛普惠投资管理有限公司第一家营业部在深圳车公庙盛大开业，公司开始了全国分店网点的战略布局。
                        </p>
                    </li>


                    <li class="clearfix">
                        <span class="time-month">7月</span>
                        <span class="graydots"></span>
                        <p>小牛普惠投资管理有限公司在深圳市卓越世纪中心成立。
                        </p>
                    </li>

                </ul>
            </div>
        </div>
        <div class="clearfix"></div>
    </div>
</div>
<!--尾部 -->


<footer>
    <div class="g-m">
        <ul class="footer-link-wrap clearfix">
            <li class="fl">
                <ul class="footer-link-list">
                    <li><a href="https://www.xnph66.com/company">关于我们</a></li>
                    <li><a href="https://www.xnph66.com/company">公司介绍</a></li>
                    <li><a href="https://www.xnph66.com/newslistrows4">新闻公告</a></li>
                    <li><a href="http://sc.hotjob.cn/wt/neocapital/web/index/social" target="_blank">加入我们</a></li>
                    <li><a href="https://www.xnph66.com/web_map">网站地图</a></li>
                </ul>
            </li>
            <li class="fl">
                <ul class="footer-link-list">
                    <li><a href="https://www.xnph66.com/product1">产品介绍</a></li>
                    <li><a href="https://www.xnph66.com/product/xnwmd">微秒贷</a></li>
                    <li><a href="https://www.xnph66.com/product/xnjyd">精英贷</a></li>
                    <li><a href="https://www.xnph66.com/product/xngxd">工薪贷</a></li>
                </ul>
            </li>
            <li class="fl">
                <ul class="footer-link-list">
                    <li><a href="javascript:void(0)">客户服务</a></li>
                    <!-- <li><a href="javascript:void(0)" id="footer-BizQQWPA">在线客服</a></li> -->
                    <li><a href="https://www.xnph66.com/branch/1">在线预约</a></li>
                    <li><a href="https://www.xnph66.com/question1">常见问题</a></li>
                    <li><a href="https://www.xnph66.com/user_feedback">用户反馈</a></li>
                </ul>
            </li>
            <li class="fl">
                <img class="qrcode" style="padding:0 5px" src="https://www.xnph66.com/HomePage/resources/xnph/images/erweima.png">
            </li>
            <li class="fl">
                <ul class="footer-link-list">
                    <li>客服专线</li>
                    <li class="footer-tel m-xs">
                        <img class="footer-icon" src="https://www.xnph66.com/HomePage/resources/xnph/images/footer_icon.png">
                        <span style="font-size: 34px;">400-8730-666</span><br>
                        <span class="pdl47 txt-sty-s">周一~周五:　9:00-18:00</span><br>
                        <span class="pdl47 txt-sty-s">(周六~周日公众假期休息)</span>
                    </li>
                    <!-- <li class="footer-qq"><img class="footer-icon" src="https://www.xnph66.com/HomePage/resources/xnph/images/footer_icon.png"><a href="javascript:;"><img  id="footer-img-BizQQWPA" src="https://www.xnph66.com/HomePage/resources/xnph/images/qq.png"></a> </li> -->
                </ul>
            </li>
        </ul>
    </div>
    <div class="clearfix copyright">
        <p>Copyright © 深圳市小牛普惠投资管理有限公司 备案号：粤ICP备16000223号-1</p>
        <!-- <div class="secure-link-area">
        <a href="javascript:vrsn_splash();" rel="nofollow" target="_blank" title="SSL" class="icon-seal"><img src="https://www.xnph66.com/HomePage/resources/xnph/images/vseal.gif" alt="SSL"></a>
        <div id="Symantec">
            <script type="text/javascript" src="https://seal.websecurity.norton.com/getseal?host_name=*.xnph66.com&amp;size=S&amp;use_flash=NO&amp;use_transparent=NO&amp;lang=zh_cn"></script>
        </div>
    </div> -->
        <script type="text/javascript">
            /*var sopener;
                                                                                                                                                                                                                                    function getSeal(){
                                                                                                                                                                                                                                        var url = 'https://trustsealinfo.websecurity.norton.com/splash?form_file=fdf/splash.fdf&dn=uat.xnph66.com&lang=zh_cn';
                                                                                                                                                                                                                                        if (sopener && !sopener.closed) {
                                                                                                                                                                                                                                            sopener.focus();
                                                                                                                                                                                                                                        } else {
                                                                                                                                                                                                                                            tbar = "location=yes,status=yes,resizable=yes,scrollbars=yes,width=560,height=500";
                                                                                                                                                                                                                                            var sw = window.open(url, 'VRSN_Splash', tbar);
                                                                                                                                                                                                                                            if (sw) {
                                                                                                                                                                                                                                                sw.focus();
                                                                                                                                                                                                                                                sopener = sw;
                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                        }
                                                                                                                                                                                                                                    }*/
        </script>

    </div>
</footer>

<!--贷款协议-->
<div class="modal fade" id="myModal_xy" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
                    ×
                </button>
                <h4 class="modal-title" id="myModalLabel" style="font-size:18px;">
                    小牛普惠借款须知
                </h4>
            </div>
            <div class="modal-body" style="font-size:14px; text-indent:2em">
                <p>请确保您提供的申请资料是正确、完整、真实、合法且有效的，同意并授权深圳市小牛普惠投资管理有限公司（下称"我司"）从事以下活动：</p>
                <p>1、 向任何第三方（包括但不限于中国人民银行个人信用信息基础数据库、其他依法成立的征信机构、文件出具机构、其他金融机构）多次查询、复核您的财产、资信、信用信息以此项作为借款申请及供我司审查及后续管理之用，包括但不限于您其后向我司以电话、网络、传真、或其他我司认可之途径再次申请借款。您明白及同意我司进行该借款事宜审查，以判断您的借款额应否取消、增加、减少或维持不变。</p>
                <p>2、 向任何第三方（包括但不限于中国人民银行个人信用信息基础数据库及其他依法成立的征信机构、其他金融机构）获得您的信用报告（包括但不限于个人基本信息、个人信用报告、个人信借交易记录等反映个人信用状况的其他相关信息）并向该等机构上传反映您的信用信息和其他相关信息。</p>
                <p>3、 有绝对的权利批准或拒绝您的借款申请而毋庸置疑须提出任何理由及退回您的相关附件。收集有关您的资料以作申请借款之用及您已获我司通知您可查阅及更正。
                </p><p>
            </p><p>4、 将您的所有资料在我司内部共享。</p>
                <p>您已详细阅读以上内容，理解并能予以遵守以上条款约定。</p>

            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">确认</button>
            </div>
        </div>
    </div>
</div>
<!--/贷款协议-->

<!-- Small modal -->
<!-- Modal -->
<div class="modal fade" id="msg-send-img-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
                <h4 class="modal-title" id="myModalLabel">输入图形验证码</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal xn-form">
                    <div class="form-group ">
                        <label class="control-label w-sm m-r-md">图形验证码</label>
                        <div class="w-xn-xl i-b">
                            <input type="text" class="form-control i-b" style="width: 180px;margin-right:11px;" name="captcha" id="txt-msg-code" placeholder="请输入图形验证码" onkeydown="if(event.keyCode==13){return false;}" onkeyup="if(event.keyCode==13){return false;}" onkeypress="if(event.keyCode==13){return false;}">
                            <img id="msgcode-img" src="/HomePage/code/getCode" style="width: 80px;height: 38px;float:right;">
                        </div>
                        <span class="text-orange m-l-sm xn-error "></span>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                <button type="button" class="btn btn-primary" id="btn-send-code-with-img">发送短信验证码</button>
            </div>
        </div>
    </div>
</div>




<script type="text/javascript" src="https://www.xnph66.com/HomePage/resources/dist/js/about.js?v=2.1.2"></script>


<script type="text/javascript" src="https://www.xnph66.com/HomePage/resources/dist/js/history.js?v=2.1.2"></script>





<script type="text/javascript">
    $('.selectpicker').on('rendered.bs.select', function(){

        $(this).parent().parent().find('.bootstrap-select-replace').remove();

    });
</script>







<script type="text/template" id="history-list-tpl">
    <li class="clearfix">
        <span class="graydots"></span>
        <p class="text-orange">{%= years[0] + 1 %}年小牛普惠在路上，再出发</p>
    </li>
    {% for(var i in years){ var year = years[i]; var histories = historyList[year]; %}
    <li class="clearfix">
        <span class="yellowcircle"></span>
        <p class="time-year">{%=year%}年</p>
    </li>
    {% for(var j in histories){ var history = histories[j]; var month = parseInt(history.historyTime.toString().substring(5,7)); %}

    <li class="clearfix">
        <span class="time-month">{%= month %}月</span>
        <span class="graydots"></span> {% if(!_.isEmpty(history.imgUrl)){ %}
        <img class="fl m-r-md w-sm" src="{%= IMG_SERVER_URL %}{%= history.imgUrl %}"> {% } %}
        <p>{%= history.content %}</p>
    </li>
    {% } %} {% } %}
</script>



</body></html>