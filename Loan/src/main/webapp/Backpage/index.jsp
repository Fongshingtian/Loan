<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>LarryCMS后台管理</title>
    <meta name="renderer" content="webkit">
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">
    <meta name="apple-mobile-web-app-status-bar-style" content="black">
    <meta name="apple-mobile-web-app-capable" content="yes">
    <meta name="format-detection" content="telephone=no">
    <!-- load css -->
    <link rel="stylesheet" type="text/css" href="common/layui/css/layui.css" media="all">
    <link rel="stylesheet" type="text/css" href="common/global.css" media="all">
    <link rel="stylesheet" type="text/css" href="css/adminstyle.css" media="all">


</head>
<body>
<div class="layui-layout layui-layout-admin" id="layui_layout">
    <!-- 顶部区域 -->
    <div class="layui-header header header-demo">
        <div class="layui-main">
            <!-- logo区域 -->
            <div class="admin-logo-box">
                <a class="logo" href="http://www.kuxuebao.net" title="logo">后台管理系统</a>
                <div class="larry-side-menu">
                    <i class="fa fa-bars" aria-hidden="true"></i>
                </div>
            </div>
            <!-- 顶级菜单区域 -->
            <div class="layui-larry-menu">
                <ul class="layui-nav clearfix">
                    <li class="layui-nav-item layui-this">
                        <a href="javascirpt:;"><i class="iconfont icon-wangzhanguanli"></i>内容管理</a>
                    </li>
                    <li class="layui-nav-item">
                        <a href="javascirpt:;"><i class="iconfont icon-weixin3"></i>微信公众</a>
                    </li>
                    <li class="layui-nav-item">
                        <a href="javascirpt:;"><i class="iconfont icon-ht_expand"></i>扩展模块</a>
                    </li>
                </ul>
            </div>
            <!-- 右侧导航 -->
            <ul class="layui-nav larry-header-item">
                <li class="layui-nav-item">
                    账户名：广州丹奇日化
                </li>
                <li class="layui-nav-item first">
                    <a href="javascript:;">
                        <cite>默认站点</cite>
                        <span class="layui-nav-more"></span>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="">站点1</a>
                        </dd>
                        <dd>
                            <a href="">站点2</a>
                        </dd>

                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;" id="lock">
                        <i class="iconfont icon-diannao1"></i>
                        锁屏</a>
                </li>
                <li class="layui-nav-item">
                    <a href="login.html">
                        <i class="iconfont icon-exit"></i>
                        退出</a>
                </li>
            </ul>
        </div>
    </div>
    <!-- 左侧侧边导航开始 -->
    <div class="layui-side layui-side-bg layui-larry-side" id="larry-side">
        <div class="layui-side-scroll" id="larry-nav-side" lay-filter="side">

            <!-- 左侧菜单 -->
            <ul class="layui-nav layui-nav-tree">
                <li class="layui-nav-item layui-this">
                    <a href="javascript:;" data-url="main.html">
                        <i class="iconfont icon-home1" data-icon='icon-home1'></i>
                        <span>后台首页</span>
                    </a>
                </li>
                <!-- 个人信息 -->
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-jiaoseguanli"></i>
                        <span>我的面板</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="personInfo.jsp">
                                <i class="iconfont icon-geren1" data-icon='icon-geren1'></i>
                                <span>个人信息</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="changepwd.jsp">
                                <i class="iconfont icon-iconfuzhi01" data-icon='icon-iconfuzhi01'></i>
                                <span>修改密码</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="myloginfo.jsp">
                                <i class="iconfont icon-piliangicon" data-icon='icon-piliangicon'></i>
                                <span>日志信息</span>
                            </a>
                        </dd>
                    </dl>
                </li>
                <!-- 用户管理 -->
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-jiaoseguanli2"></i>
                        <span>表格表单</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="table.jsp">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>有线条表格</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="table_1.jsp">
                                <i class="iconfont icon-jiaoseguanli4" data-icon='icon-jiaoseguanli4'></i>
                                <span>无线条输入增加内容框</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="myloginfo.jsp">
                                <i class="iconfont icon-quanxian2" data-icon='icon-quanxian2'></i>
                                <span>无线条表格</span>
                            </a>
                        </dd>
                    </dl>
                </li>
                <%--产品介绍--%>
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-jiaoseguanli2"></i>
                        <span>产品管理</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="/produce/list">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>产品介绍</span>
                            </a>
                        </dd>

                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-jiaoseguanli2"></i>
                        <span>首页管理</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="/t2/test2">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>轮播图管理</span>
                            </a>
                        </dd>
                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-jiaoseguanli2"></i>
                        <span>信息披露</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=1">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>平台介绍</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/dis/updhonor">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>资质荣誉</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/dis/updcollaborator">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>合作伙伴</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=2">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>重大事项</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=3">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>联系我们</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=4">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>备案信息</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=5">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>消费金融</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=6">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>信贷金融</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=7">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>收费标准</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=8">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>保障方式</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/t2/ites1?id=9">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>风险管理</span>
                            </a>
                        </dd>
                        <dl class="layui-nav-child">
                            <dd>
                                <a href="javascript:;" data-url="/wlk/newsList2">
                                    <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                    <span>公告管理</span>
                                </a>
                            </dd>
                        </dl>
                        <dl class="layui-nav-child">
                            <dd>
                                <a href="javascript:;" data-url="/wlk/newsList2">
                                    <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                    <span>公告管理</span>
                                </a>
                            </dd>
                        </dl>
                        <dl class="layui-nav-child">
                            <dd>
                                <a href="javascript:;" data-url="/wlk/backlawlist">
                                    <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                    <span>法律法规</span>
                                </a>
                            </dd>
                        </dl>
                    </dl>
                </li>
                <%--个人账户管理--%>
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-jiaoseguanli2"></i>
                        <span>个人账户管理</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="/myusers/list">
                                <i class="iconfont icon-yonghu1" data-icon='icon-yonghu1'></i>
                                <span>我的账户信息</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/myusers/urgentlist">
                                <i class="iconfont icon-jiaoseguanli4" data-icon='icon-jiaoseguanli4'></i>
                                <span>紧急联系人</span>
                            </a>
                        </dd>

                        <dd>
                            <a href="javascript:;" data-url="/myusers/yhklist">
                                <i class="iconfont icon-quanxian2" data-icon='icon-quanxian2'></i>
                                <span>银行卡管理</span>
                            </a>
                        </dd>

                        <dd>
                            <a href="javascript:;" data-url="/myusers/recordlsit">
                                <i class="iconfont icon-jiaoseguanli4" data-icon='icon-jiaoseguanli4'></i>
                                <span>资金记录</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/myusers/messgelist">
                                <i class="iconfont icon-quanxian2" data-icon='icon-quanxian2'></i>
                                <span>我的消息</span>
                            </a>
                        </dd>

                        <dd>
                            <a href="javascript:;" data-url="/myusers/accountlist">
                                <i class="iconfont icon-quanxian2" data-icon='icon-quanxian2'></i>
                                <span>个人账户信息</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/myusers/jflist">
                                <i class="iconfont icon-quanxian2" data-icon='icon-quanxian2'></i>
                                <span>积分记录</span>
                            </a>
                        </dd>

                        <dd>
                            <a href="javascript:;" data-url="/myusers/friendlist">
                                <i class="iconfont icon-jiaoseguanli4" data-icon='icon-jiaoseguanli4'></i>
                                <span>我的好友</span>
                            </a>
                        </dd>

                        <dd>
                            <a href="javascript:;" data-url="/myusers/friendgetlist">
                                <i class="iconfont icon-jiaoseguanli4" data-icon='icon-jiaoseguanli4'></i>
                                <span>好友赚取记录</span>
                            </a>
                        </dd>

                        <dd>
                            <a href="javascript:;" data-url="/myusers/yhjlist">
                                <i class="iconfont icon-lanmuguanli" data-icon='icon-yonghu1'></i>
                                <span>红包优惠劵</span>
                            </a>
                        </dd>
                    </dl>
                </li>

                <%--申请审批--%>
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-wenzhang1"></i>
                        <span>审批管理</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="/back/loan">
                                <i class="iconfont icon-lanmuguanli" data-icon='icon-lanmuguanli'></i>
                                <span>第一次审批</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/back/twoloan">
                                <i class="iconfont icon-lanmuguanli" data-icon='icon-lanmuguanli'></i>
                                <span>第二次审批</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/back/shistory">
                                <i class="iconfont icon-lanmuguanli" data-icon='icon-lanmuguanli'></i>
                                <span>审批历史</span>
                            </a>
                        </dd>
                    </dl>
                </li>
                2019-04-29 10:04:50
                <!-- 内容管理 -->
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-wenzhang1"></i>
                        <span>内容管理</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="404.jsp">
                                <i class="iconfont icon-lanmuguanli" data-icon='icon-lanmuguanli'></i>
                                <span>404页面</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;">
                                <i class="iconfont icon-wenzhang2" data-icon='icon-wenzhang2'></i>
                                <span>所有档案列表</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;">
                                <i class="iconfont icon-icon1" data-icon='icon-icon1'></i>
                                <span>待审核的档案</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;">
                                <i class="iconfont icon-word" data-icon='icon-word'></i>
                                <span>我发布的文档</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;">
                                <i class="iconfont icon-pinglun1" data-icon='icon-pinglun1'></i>
                                <span>评论管理</span>
                            </a>
                        </dd>

                        <dd>
                            <a href="javascript:;">
                                <i class="iconfont icon-tags1" data-icon='icon-tags1'></i>
                                <span>TAGS管理</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;">
                                <i class="iconfont icon-huishouzhan1" data-icon='icon-huishouzhan1'></i>
                                <span>内容回收站</span>
                            </a>
                        </dd>
                    </dl>
                </li>

                <!-- 会员管理 -->
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-m-members"></i>
                        <span>营业网点</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="/businessOutlets">
                                <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                <span>营业网点</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/onlineBookings">
                                <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                <span>在线预约</span>
                            </a>
                        </dd>
                    </dl>
                </li>
                <li class="layui-nav-item">
                    <a href="javascript:;">
                        <i class="iconfont icon-m-members"></i>
                        <span>会员中心</span>
                        <em class="layui-nav-more"></em>
                    </a>
                    <dl class="layui-nav-child">
                        <dd>
                            <a href="javascript:;" data-url="/vip/BackStage/task/1">
                                <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                <span>成长任务</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/vip/BackStage/growth_value/1">
                                <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                <span>成长经历</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/vip/BackStage/point_mall/1">
                                <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                <span>积分商城</span>
                            </a>
                        </dd>
                        <dd>
                            <a href="javascript:;" data-url="/vip/BackStage/mall_exchange/1">
                                <i class="iconfont icon-zhuti" data-icon='icon-zhuti'></i>
                                <span>商城兑换</span>
                            </a>
                        </dd>
                    </dl>
                </li>
            </ul>
        </div>
    </div>

    <!-- 左侧侧边导航结束 -->
    <!-- 右侧主体内容 -->
    <div class="layui-body" id="larry-body" style="bottom: 0;border-left: solid 2px #2299ee;">
        <div class="layui-tab layui-tab-card larry-tab-box" id="larry-tab" lay-filter="demo" lay-allowclose="true">
            <div class="go-left key-press pressKey" id="titleLeft" title="滚动至最右侧"><i
                    class="larry-icon larry-weibiaoti6-copy"></i></div>
            <ul class="layui-tab-title">
                <li class="layui-this" id="admin-home"><i class="iconfont icon-diannao1"></i><em>后台首页</em></li>
            </ul>
            <div class="go-right key-press pressKey" id="titleRight" title="滚动至最左侧"><i
                    class="larry-icon larry-right"></i></div>
            <ul class="layui-nav closeBox">
                <li class="layui-nav-item">
                    <a href="javascript:;"><i class="iconfont icon-caozuo"></i> 页面操作</a>
                    <dl class="layui-nav-child">
                        <dd><a href="javascript:;" class="refresh refreshThis"><i class="layui-icon">&#x1002;</i>
                            刷新当前</a></dd>
                        <dd><a href="javascript:;" class="closePageOther"><i class="iconfont icon-prohibit"></i>
                            关闭其他</a></dd>
                        <dd><a href="javascript:;" class="closePageAll"><i class="iconfont icon-guanbi"></i> 关闭全部</a>
                        </dd>
                    </dl>
                </li>
            </ul>
            <div class="layui-tab-content" style="min-height: 150px; ">
                <div class="layui-tab-item layui-show">
                    <iframe class="larry-iframe" data-id='0' src="main.jsp"></iframe>
                </div>
            </div>
        </div>
    </div>
    <!-- 底部区域 -->
    <div class="layui-footer layui-larry-foot" id="larry-footer">
        <div class="layui-mian">
            <p class="p-admin">
                <span>2017 &copy;</span>
                广州普金计算机科技股份有限公司,版权所有
            </p>
        </div>
    </div>
</div>
<!-- 加载js文件-->
<script type="text/javascript" src="common/layui/layui.js"></script>
<script type="text/javascript" src="js/larry.js"></script>
<script type="text/javascript" src="js/index.js"></script>
<!-- 锁屏 -->
<div class="lock-screen" style="display: none;">
    <div id="locker" class="lock-wrapper">
        <div id="time"></div>
        <div class="lock-box center">
            <img src="images/userimg.jpg" alt="">
            <h1>admin</h1>
            <duv class="form-group col-lg-12">
                <input type="password" placeholder='锁屏状态，请输入密码解锁' id="lock_password" class="form-control lock-input"
                       autofocus name="lock_password">
                <button id="unlock" class="btn btn-lock">解锁</button>
            </duv>
        </div>
    </div>
</div>

</body>
</html>